 ...
 * (e.g. jumpinh, going up stairs). Because the computer operates using delta 
...
public class RootMotionComputer : MonoBehaviour
{
	...
	public Vector3 pelvisForwardAxis = Vector3.forward; // its local axis specifying the forward direction
	...
	{
		// ortho-normalize pelvis axes for drawing
		Vector3 pForward = pelvis.TransformDirection(pelvisForwardAxis);
		Vector3 pRight = pelvis.TransformDirection(pelvisRightAxis);
		Vector3 pUp = Vector3.Cross(pForward, pRight);
		pForward = Vector3.Cross(pRight, pUp);
		
		// draw pelvis axes
		Debug.DrawRay(pelvis.position, pForward * debugGizmoSize, Color.blue);
		Debug.DrawRay(pelvis.position, pRight * debugGizmoSize, Color.red);
		Debug.DrawRay(pelvis.position, pUp * debugGizmoSize, Color.green);
